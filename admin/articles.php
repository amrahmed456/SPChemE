<?php
	...
	*/
	...
	if(isset($_SESSION['user_type'])){
		...
		<?php
			// getting users data
			$stmt = $db->prepare("SELECT 					articles.approval,articles.item_id,articles.name,articles.description,articles.add_date, articles.cat_id, categories.name AS cat_name FROM articles JOIN categories ON articles.cat_id = categories.cat_id ORDER BY articles.add_date DESC");
			...
			<form action="articles.php?do=insert" enctype="multipart/form-data" method="POST"class="new-item-form">
					...
					<div class="form-group row">
						<label for="userNameEdit" class="col-sm-2">Content</label>
						<div  class="col-sm-10 col-md-8">
							<textarea id="editor" class="form-control"placeholder="Item Description..." name="content" required=""></textarea>
						</div>
					</div>
				...
				if($_SERVER['REQUEST_METHOD'] == 'POST'){
					...
					if($count > 0){
						...
						if($countErrors == 0){
							...
							$stmt = $db->prepare("INSERT INTO articles (name,description,content,add_date,cat_id,image,approval) VALUES (?,?,?,NOW(),? , ?,?)");
							$stmt->execute(array($item_name , $description, $content , $cat_id , $image_name , "1"));
						...
						if($countErrors == 0){
							...
							$stmt2 = $db->prepare("INSERT INTO articles (name,description,content,add_date,cat_id,image,approval) VALUES (?,?,?,NOW(),? ,?, ?)");
							$stmt2->execute(array($item_name , $description , $content , $cat_id , $image_name, "1"));
