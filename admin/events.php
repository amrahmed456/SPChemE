			...
			<div class="container">
				...
				?>
					...
					<div class="form-group row">
						...
						<div  class="event-cat position-relative col-sm-10  col-md-8">
				...
				if($_SERVER['REQUEST_METHOD'] == 'POST'){
					...
					if($countErrors == 0){
						...
						if($num_photos == 0){
							...
							$stmt = $db->prepare("INSERT INTO events (name,description,content,date,end_date,facebook , registeration,location,category , images , status , phone , email,topics,cover) VALUES (? ,?,? ,? ,? ,? ,?,?,?,? ,? ,? ,? ,?, ?)");
							$stmt->execute(array($event_name , $description,$content , $date , $end_date , $facebook , $registeration, $location , $category , $photos , $status , $phone , $email , $topics, $image_name));
							if($stmt->rowCount() > 0){
