$(document).ready(function(){
	...
	*/
	// for real time category list while typing in items page the category name in articles page
	var select_pos = 0;
	$("input[name='cat_name'],.event-cat input[name='category']").on({
		"keydown": function(e){
			if( e.which == 13 ){
			// user clicked enter
		...
		if(e.which == 40){
			...
			if(cat_name.length > 0){
				...
				if($(this).attr("name") == 'cat_name'){
					// request categories for articles page
					url_file = 'search_cat.php';
				}else if($(this).attr("name") == 'category'){
					url_file = 'search_cat_event.php';
				}
				$.ajax({
					url:	url_file,
					method:	"POST",
	...
	$("body").on("click",".cat-result p",function(){
		...
		$("input[name='category']").val(select_text);
		$(".cat-result").addClass("d-none");
